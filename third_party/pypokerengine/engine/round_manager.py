...
from third_party.pypokerengine.engine.action_checker import ActionChecker
from third_party.pypokerengine.engine.game_evaluator import GameEvaluator
from third_party.pypokerengine.engine.message_builder import MessageBuilder
...
class RoundManager:
  ...
  def __start_street(self, state):
    table = state["table"]
    # Heads-up exception: the button/small blind acts first preflop, while the
    # big blind acts first on every postflop street.
    heads_up_postflop = (
        len(table.seats.players) == 2
        and state["street"] != Const.Street.PREFLOP
    )
    start_pos = table.dealer_btn if heads_up_postflop else table.sb_pos() - 1
    next_player_pos = table.next_ask_waiting_player_pos(start_pos)
  ...
  def __showdown(self, state):
    table = state["table"]
    pots = GameEvaluator.create_pot(table.seats.players)
    winners, hand_info, prize_map = GameEvaluator.judge(state["table"])
    self.__prize_to_winners(state["table"].seats.players, prize_map)
    state["round_result"] = {
        "winner_uuids": [player.uuid for player in winners],
        "hand_info": hand_info,
        "prize_map": {
            table.seats.players[index].uuid: prize
            for index, prize in prize_map.items()
        },
        "pots": [
            {
                "amount": pot["amount"],
                "eligible_uuids": [
                    player.uuid for player in pot["eligibles"]
                ],
                "winner_uuids": [
                    player.uuid for player in GameEvaluator.find_winners(
                        table.get_community_card(), pot["eligibles"]
                    )
                ],
            }
            for pot in pots if pot["amount"] > 0
        ],
    }
    result_message = MessageBuilder.build_round_result_message(state["round_count"], winners, hand_info, state)
    # This integration runs exactly one hand. Preserve the board, hole cards,
    # and contribution audit instead of resetting for another round.
    state["street"] += 1
