...
class HumanIdentityApiTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_page_has_all_games_and_no_identity_or_room_join_inputs(self):
        ...
        for game_type in (
            ...
            "connect4", "banqi", "checkers", "chess", "dots_boxes", "jungle", "junqi", "xiangqi",
        ):
        ...
        self.assertIn("function renderRecentChat(timeline = [])", script.text)
        self.assertIn("function recentSpeechEvents(timeline = []", script.text)
        self.assertIn("event.is_public !== false", script.text)
        self.assertIn('speechSenderRole(event) !== "system"', script.text)
