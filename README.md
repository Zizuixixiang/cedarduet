...
- `yahtzee`：2–6 人快艇骰子，支持系统 NPC、公开计分卡与持久掷骰
- `jungle`：7×9 斗兽棋
- `xiangqi`：9 路 10 行象棋，固定人类与真实绑定小机双人对局

井字棋、五子棋、黑白棋、四子连珠、斗兽棋和象棋继续严格双人。点格棋权威声明
`allowed_player_counts=(2,3,4)`，吹牛骰子与快艇骰子声明 `(2,3,4,5,6)`；三者均
复用多人/NPC 框架。快艇骰子暂不支持筹码下注。
...
每人使用 5 枚六面骰，每回合至少掷一次、最多掷三次；后两次可以按位置保留任意
骰子并只重掷其余骰子。玩家随后填写一个尚未使用的类别：一点至六点、三条、四条、
葫芦、小顺、大顺、快艇/五同或机会。三条、四条和机会按骰子总和，葫芦 25 分、
小顺 30 分、大顺 40 分、快艇 50 分；上半区达到 63 分加 35 分。不符合组合的类别
记 0 分，也可显式划掉任意未用类别记 0 分。每类只能填写一次，全员各填满 13 类后
总分最高者获胜；最高分多人并列按和局处理，并列名单按稳定座位顺序展示。

第一版不实现重复快艇额外 100 分和 Joker 规则。掷骰由服务端生成并立即保存在
`board_state.dice` 与原始投掷记录中，刷新、重连和状态投影都不会重新掷骰。公开状态
包含当前骰子、全员计分卡、总分和当前合法类别预估；系统 NPC 另从权威合法行动列表
选择保留、重掷或计分。`supports_stakes=False`，只能创建娱乐局。
...
- 参与者真源用 `participant_kind` 区分 `human`、`bound_machine` 和
  ...
  不会接管中途离开的席位。点格棋、吹牛骰子与快艇骰子声明 `supports_npcs`。
...
- 插件通过 `participant_summary` 只提供至多四项公开标量元数据；通用座位卡负责
  头像、姓名、座位、行动高亮和状态；点格棋补得分，吹牛骰子补剩余骰数，快艇骰子
  补当前总分和已填类别数。
