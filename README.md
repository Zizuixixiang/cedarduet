...
当前 25 款中只有 `yahtzee`（快艇骰子）固定为 0 筹码娱乐局；其余游戏均可按 catalog 创建带 `stake` 的房间。多数游戏采用明确零和策略：开火车按每名败者一个 stake；斗地主按最终叫分/炸弹倍率在地主与两农民间结算；掼蛋按两人队伍胜负结算；军棋、围棋走双人 ±stake；麻将按自摸或点炮/抢杠来源结算；炸金花按本局实际下注单位×stake；德州扑克把 stake 视为每席完整买入，终局按最终内部栈比例分配真实总买入池。21点是唯一明确 opt-in 的非零和例外：每席独立对虚拟庄家按胜 `+stake`、负 `-stake`、推和 `0` 结算。
...
其他游戏仍执行零和校验。认输席固定为 `-stake`，其余席继续行动，整桌只在真正终局
一次性结算；NPC 仍只记录逻辑 delta，不创建永久钱包。
...
- 终局结算使用同一全局钱包与流水；普通双人仍为胜方 `+stake`、负方 `-stake`、和棋 0，特殊游戏以房间 `settlement_deltas` 为准。认输按输结算，重复读取终局不会重复记账；`leave` 保持独立的房间生命周期语义，不自动冒充认输。
- 多人筹码默认禁用；具体多人插件必须显式声明支持并给出自己的 settlement
  deltas（完整覆盖每名参与者、整数且默认总和为 0），框架原子幂等结算，绝不会
  推断“赢家拿走其余人的 stake”。只有 21点按游戏级显式开关允许非零和。
- 固定人数认输按具体阵营/包赔规则：斗地主定地主后认输者所在阵营判负并使用当前
  multiplier；未定地主时认输者 `-2×stake`、其余各 `+stake`。掼蛋认输者所在队判负；
  麻将认输者包赔 `-3×stake`、其余各 `+stake`；中国跳棋六人桌认输后不进入非法五人局，
  认输者 `-5×stake`、其余各 `+stake`。
