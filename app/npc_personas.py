...
ALLOWED_SUPPORT_FILES = {"README.md", "_schema.json"}
...
def _parse_persona(path: Path) -> NpcPersona:
    ...
    if not isinstance(raw, dict) or set(raw) != {"id", "display_name", "persona"}:
        raise PersonaConfigError(
            f"NPC 人设 {path.name} 必须且只能包含 id/display_name/persona"
        )
    ...
    return NpcPersona(persona_id, display_name, persona)
...
def select_personas(
    ...
    *,
    ...
    if count > 2:
        raise PersonaConfigError("每局最多补入 2 名 NPC")
