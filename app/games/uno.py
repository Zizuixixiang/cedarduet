...
class Uno(GamePlugin):
    ...
    rules_text = (
        ...
        "避免把功能牌效果无归属地施加给首位玩家。轮到自己时可出同颜色、同数字或同"
        "功能符号的牌，也可出 Wild；无牌可出或主动不出时摸 1 张，只有这张刚摸到的牌"
        "可在当前回合立即打出，否则回合结束。不采用 +2/+4 叠加。Skip 跳过下一位；"
        ...
        "漏报者摸 2，之后窗口关闭。先出完手牌者获胜；最后一张若为 Draw Two，会先让"
        "目标玩家摸牌再终局；最后一张若为 Wild Draw Four，必须等下一位完成不质疑或"
        "质疑结算后才终局，质疑成功则原出牌者因摸回 4 张而不能获胜。摸牌堆耗尽时保留"
