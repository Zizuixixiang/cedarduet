...
class Banqi(GamePlugin):
    ...
    rules_text = (
        "使用一副 32 子中国象棋随机扣置于 4×8 棋盘。首位玩家第一次翻出的棋子颜色"
        "决定其阵营，另一方自动归属另一颜色；每回合可翻一枚暗子，或移动己方已经"
        "翻开的棋子。普通棋每次走相邻一格，吃子遵循帅/将＞士＞象＞车＞马＞炮＞兵/卒，"
        "同级可互吃；兵/卒可吃帅/将，帅/将不能吃兵/卒。炮只能移动到相邻空位；吃子时"
        "须与目标同行或同列并隔恰好一个棋子作炮架；炮架可以是明子或暗子，目标必须是"
        "已经翻开的敌子。一方所有棋子被吃光，或轮到其行动却没有任何合法行动时判负。"
        "为防止残局无限循环，连续 40 手既未翻子也未吃子时判和。"
    )
    move_format = (
