...
class ChineseCheckers(GamePlugin):
    ...
    rules_text = (
        "中国跳棋使用标准 121 孔六角星棋盘，支持 2、3、4、6 人（不支持 5 人），"
        "每人 10 颗弹珠。2 人使用一对对角营，3 人隔一角入座，4 人使用两对对角营，"
        "6 人六角全开；目标营始终是起始营正对面的三角。每回合只移动一颗：可以沿六个"
        "方向走到相邻空孔并立即结束，或只用跳跃跨过相邻任意阵营的一颗弹珠，落到同一直线"
        "紧邻其后的空孔；跳跃可连续任意次，不吃子，同一跳跃链不得重复落点，也不能把普通"
        "一步与跳跃混用。自己的起始营和目标营可停留；其它四个角营不可作为回合终点，但"
        "连续跳的中间落点可穿过。弹珠一旦进入自己的目标营便不能离开，只能在目标营内部"
        "移动或跳跃。通常先把自己的 10 子全部送入目标营者获胜。为防止对手故意把原始棋"
        "长期留在其起始营（即你的目标营）拖死终局，本局采用 anti-spoiling：目标营十孔"
        "全部被占、其中至少一颗是你的棋，且其余阻挡棋只属于该营的原始拥有者时，也立即"
        "判你获胜；这既不会在开局误判，也不能用第三方棋冒充有效阻挡。多人下注局由唯一"
        "赢家获得其余每席各一份 stake。"
    )
    move_format = (
