...
class AeroplaneChess(GamePlugin):
    ...
    rules_text = (
        "标准中国飞行棋，支持 2–4 人，每色 4 架飞机。只有掷出 6 才能把一架飞机从"
        "机场移到安全起飞区；起飞动作不再额外前进 6 格。已经起飞的飞机按骰点沿 52 格"
        "公共环线精确前进。掷出 6 并完成本次动作后可继续掷骰；若无飞机可动也由服务端"
        "自动处理续掷。连续第三个 6 不允许移动，并把本轮前两个 6 实际移动过的飞机送回"
        "机场，随后立即结束该玩家回合；此前造成的碰撞不撤销。落在对手占据的普通环线"
        "格时，该格所有对手机全部回机场。己方飞机可同格，但每次只移动所选的一架，不"
        "组成叠机单位。落在己方颜色的普通跳跃格会自动前跳 4 格；每色相对路线第 21 格"
        "是特殊跨盘飞跃格，直达第 33 格，从前一同色格自动跳到该格时也继续飞跃。飞跃"
        "会击落跨越点及落点的对手机；特殊飞跃落点不再连锁一次普通跳跃。所有自动落点"
        "均照常结算碰撞。绕完环线后进入己方 6 格终点航道，再以精确点数到达中心；点数"
        "超出时该飞机不可选择，不采用反弹。机场、起飞区、终点航道与中心均为安全区。"
        "首位把 4 架飞机全部送到中心的玩家获胜。不采用偶数起飞、叠机同行、点数过大"
        "反弹或其他可选规则。所有骰子由服务端生成并持久化，刷新不会重掷。"
    )
    move_format = (
